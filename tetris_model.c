...
static void place_piece(int board[][BOARD_WIDTH], int ptype, int rot, int col,
                        int row) {
  for (int i = 0; i < 4; i++) {
    int pr = PIECE_CELLS[ptype][rot][i][0] + row;
    int pc = PIECE_CELLS[ptype][rot][i][1] + col;
...
static int aggregate_height(int board[][BOARD_WIDTH]) {
  int total = 0;
  for (int c = 0; c < BOARD_WIDTH; c++) total += col_height(board, c);
  return total;
}
...
static void simulate(int model, const char* log_file, Stats* st) {
  ...
  for (int turn = 0; turn < SEQUENCE_LENGTH; turn++) {
    ...
    for (int rot = 0; rot < PIECE_NUM_ROTATIONS[ptype]; rot++) {
      ...
      for (int col = 0; col <= max_col; col++) {
        ...
        if (model == 0) {
          ...
          score = -0.510066 * ah + 0.760666 * lines - 0.356630 * holes -
                  0.184483 * bump;
        }
    ...
    printf("[%s] Turn %2d: piece=%-1s rot=%d col=%2d lines=%d\n",
           st->model_name, turn + 1, PIECE_NAMES[ptype], best_rot, best_col,
           lines);
  }
...
static void print_analysis(const Stats* g, const Stats* h) {
  ...
  printf("\nwinner: %s\n", (g->total_lines > h->total_lines)   ? "GREEDY"
                           : (h->total_lines > g->total_lines) ? "HEURISTIC"
                                                               : "TIE");
}
...
int main(void) {
  ...
  srand((unsigned)(ts.tv_nsec ^ ((unsigned long)ts.tv_sec << 20) ^
                   (unsigned)getpid()));
  generate_sequence();
