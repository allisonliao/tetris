...
/* --------------------------------- render --------------------------------- */
static void render(const char* model, int turn, const char* piece, int rotation,
                   int col, int lines_cleared,
...
int main(int argc, char* argv[]) {
  ...
  while (fgets(line, sizeof(line), f)) {
    ...
    if (strncmp(line, "MODEL ", 6) == 0) {
      ...
      printf(
          "\033[1;32m=== End of replay | Model: %s | Total lines: %d "
          "===\033[0m\n",
          model, total);
      fflush(stdout);
      break;

    } else if (state != READING_NONE && board_row < BOARD_HEIGHT &&
               len == BOARD_WIDTH) {
      // board row data
      char (*target)[BOARD_WIDTH + 1] =
          (state == READING_PLACED) ? placed : board;
